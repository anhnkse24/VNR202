...
export default function ImagePlaceholder({ src, alt, tip, aspectRatio = "aspect-[16/10]", className = "", showCaption = false }) {
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      <div className={`relative overflow-hidden rounded-xl border border-museum-red/10 bg-museum-creamDark/30 shadow-sm ${aspectRatio}`}>
        {!hasError ? (
          <img 
            src={src} 
            alt={alt} 
          ...
          <div className="w-full h-full flex flex-col justify-between p-5 bg-gradient-to-br from-museum-cream to-museum-creamDark border-2 border-dashed border-museum-gold/30 text-museum-charcoal relative">
            ...
            <div className="flex flex-col items-center justify-center flex-grow py-3">
              ...
              <p className="font-serif text-xs font-bold text-museum-charcoal/50 uppercase tracking-wider text-center line-clamp-1 px-2">
                Ảnh tư liệu
              </p>
            </div>
      ...
      {/* Styled Academic Caption beneath the image (always visible if showCaption is true) */}
      {showCaption && alt && (
        <p className="text-center font-serif text-xs sm:text-sm text-museum-charcoal/70 italic mt-1 leading-relaxed px-4">
          Ảnh tư liệu: {alt}
        </p>
      )}
    </div>
